%函数功能：
%读取arcmap数据，进行点合并，绘制路网图，保存处理后的数据
%坐标编号，坐标关系转化为点关系，根据点关系得到邻接矩阵
%根据邻接矩阵构造图
%data属性:id x1 y1 x2 y2 length
function [A,G,data,uniqueValues] = makemap(filename)
%返回值：
%A：邻接矩阵
%G：无向图
%data：原始数据的所有坐标
%uniqueValues:节点坐标对应的唯一id
%% 读取数据
% 指定要读取的文件名和工作表名
sheet = 1;  % 第一个工作表
% 使用xlsread函数读取数据
